...
function(vcf,pat.col=1,mat.col=2,child.col=3){
 ...
 #cat(sn,"\n")
 if(!all(sn%in%c(1:22,paste("chr",c(1:22),sep="")))) stop("this function intended for autosomes only") 
 st = start(vcf)
 g = geno(vcf)$GT
 ...
 ## get the index of the parental allele (note that we're only pulling
 ## the first noted allele, which will be homozygous in the final, filtered
 ## data)
